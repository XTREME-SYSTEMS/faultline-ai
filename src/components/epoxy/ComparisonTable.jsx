...
const ROWS = [
  ...
  { feature: 'Installation Time', epoxy: '2 Days', concrete: '—', tiles: '3-5 Days', paint: '1 Day' },
  ...
  { feature: '10 Year Warranty', epoxy: true, concrete: false, tiles: false, paint: false }
];

function Cell({ value }) {
  if (value === true) return <Check size={18} color={LIME} style={{ margin: '0 auto', filter: 'drop-shadow(0 1px 0 rgba(0,0,0,0.2))' }} />;
  if (value === false) return <X size={18} color="#ccc" style={{ margin: '0 auto' }} />;
  return <span style={{ fontSize: 13, color: '#555' }}>{value}</span>;
}

/**
 * Comparison table — positions MVB coating against alternatives.
  ...
  return (
    <section style={{ padding: '80px 24px', background: '#fff' }}>
      <div style={{ textAlign: 'center', marginBottom: 40 }}>
        <h2 style={{ fontSize: 36, fontWeight: 800, color: CHARCOAL, margin: 0 }}>Why MVB Beats the Rest</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14, minWidth: 600 }}>
          <thead>
            <tr style={{ background: '#f5f5f5' }}>
              ...
              <th style={{ padding: '16px', textAlign: 'center', color: LIME, fontWeight: 800, borderBottom: `3px solid ${CHARCOAL}` }}>MVB</th>
          ...
          <tbody>
            {ROWS.map((r, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={{ padding: '14px 16px', textAlign: 'center', background: '#f9ffe8', boxShadow: 'inset 2px 0 0 rgba(0,0,0,0.08)' }}><Cell value={r.epoxy} /></td>
