    ...
    <MarketingShell>
      <section className="bg-[#000000] text-white py-20">
        <div className="max-w-4xl mx-auto px-5 text-center">
          <span className="text-[#FFE566] text-xs font-bold uppercase tracking-widest">Solutions</span>
          ...
          <p className="mt-5 text-lg text-slate-300">From the first click to the fifth repeat sale, FaultLine AI is tuned to the way local service businesses win work.</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-5 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {SOLUTIONS.map(s => (
            <div key={s.t} className="p-6 rounded-2xl border border-slate-200 hover:shadow-lg transition">
              <div className="w-11 h-11 rounded-xl bg-[#FFD700]/10 text-emerald-600 flex items-center justify-center"><s.icon size={22} /></div>
        ...
        <div className="max-w-5xl mx-auto px-5 grid md:grid-cols-3 gap-6 text-center">
          {[
            ...
            <div key={x.l} className="p-8 rounded-2xl bg-white border border-slate-200">
              <div className="text-4xl font-bold text-[#FFD700]" style={{ fontFamily: "'Libre Caslon Display', serif" }}>{x.n}</div>
